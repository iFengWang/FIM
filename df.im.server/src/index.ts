...
const socketOption: Partial<socketIO.ServerOptions> = {
  path: "/im",
  serveClient: false,
  pingTimeout: 60000,
  pingInterval: 25000,
  ...
  transports: ['websocket', 'polling'] as socketIO.ServerOptions['transports'],
...
io.on("connection", (socket) => {
  ...
  socket.on("register", (data) => {
    ...
    console.log("Current sockets:", {
      ...
      mapping: Object.entries(sockets).map(([uid, s]) => ({
        uid,
        socketId: (s as socketIO.Socket).id,
        connected: (s as socketIO.Socket).connected
      }))
    });
  ...
  socket.on("join", (data) => {
    ...
    let selfAccount = users[data.roomid].filter(
      (u) => u.account.id === data.account.id
    );
  ...
  socket.on("message", (msg) => {
    ...
    try {
      if (msg.account && msg.account.id && msg.from && msg.from.id) {
        ...
        const messageForSender = { ...messageWithTimestamp, unread: false }; // 发送者看到的消息标记为已读
  ...
  socket.on("disconnect", (reason) => {
    ...
    for (const [uid, s] of Object.entries(sockets)) {
      if ((s as socketIO.Socket).id === socket.id) {
